...
export class UIController extends Component {
    ...
    @property({ tooltip: "为 true 时在 onLoad 内 createSdk 并注入 CleverSdkGlobals（默认 mock，传音选 ahagame）" })
    ...
    @property({ tooltip: "传音应用 App Key（与官方壳示例一致时可保留 5024356）" })
    ...
    @property({
        tooltip:
            "联调时开启 data-adbreak-test（测试广告）；上线务必取消勾选",
    })
    ...
    private buildGraphicsDemoUi() {
        ...
        const rightSpecs: { name: string; label: string; onClick: () => void }[] = [
            ...
            { name: "SdkTestButton", label: "SDK测试", onClick: () => this.onSdkTest() },
        ];
